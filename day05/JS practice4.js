...
for( let i = 0 ; i<=userNames.length - 1 ; i++) {
    let 찾은글자 = userNames[i].indexOf('솔')
...
//재고가 부족하면 "재고가부족합니다."를 출력합니다.

let products = ['볼펜', '노트', '지우개'];
let stock = [10, 5, 20];

let 구매상품명 = prompt ( )
let 수량 = Number ( prompt ( ) ) 
let 인덱스 = products.indexOf(구매상품명)

if ( stock[인덱스] - 수량 > 0 ) { 
    console.log ( '구매 완료!' ) 
    stock[인덱스] = stock[인덱스] - 수량
}

else { (console.log( '재고가부족합니다') ) }
...
//(1). 초기 데이터 영화 이름과 평점은 두 배열의 동일한 인덱스를 사용합니다.
//(2). 구현 조건 for 반복문을 사용하여 모든 영화를 순회합니다.
...
for ( i = 0 ; i <= movieNames.length -1 ; i++ ) {
    html = html + movieNames[i]

    for ( let 별수 = 1 ; 별수 <= 10 ; 별수++) {
        if (movieRatings[i] >= 별수 ) { html = html + "★"}
        else { html = html + "☆"}
    }
...
//(1). 초기 데이터 좌석의 상태는 '빈좌석' 또는 '예약석'으로 구성됩니다.
//(2). 구현 조건 for 반복문을 사용하여 6개의 좌석을 모두 출력합니다.
//각 좌석은 하나의  태그로 표시합니다. 좌석 상태에 따라  태그에 CSS 색상을 다르게 적용합니다.
//'빈좌석': color: blue;
//'예약석': color: red;
...
for ( let i = 0 ; i <= seatStatus.length - 1 ; i++ ) {
    ...
    if (seatStatus[i] == '빈좌석') {
        html = html + '<span style="color: blue;">' + seatStatus[i] + '</span>'
    }
    else {
        html = html + '<span style="color: red;">' + seatStatus[i] + '</span>'
    }
...
//(3). 구현 조건
//for 반복문을 사용하여 모든 차량의 데이터를 순회합니다. 각 차량의 주차 시간에 맞춰 최종 요금을 계산합니다.
...
//(힌트)
//기본 시간(30분)을 초과한 시간을 계산하고, parseInt() 함수를 사용하여 10분 단위로 버림 처리하면 
//추가 요금 단위를 쉽게 계산할 수 있습니다.
