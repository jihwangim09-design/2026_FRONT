...
// 1. 정수
console.log ( 10 )
console.log ( typeof 10 ) // typeof 자료, 해당 자료의 타입명 
...
console.log( '안녕' , '하세요' )
console.log( '안녕' + '하세요' )
console.log( `안녕 ${ 3 }`)     // 문자열 탬플릿 ? 문자열내 코드 포함
...
// [4] undefine, null   ,   컴퓨터는 0 vs ' '(공백) vs null(참조가없다) vs undefined(정의가없다) 모두 다르게 취급
...
// [5] 배열 , 순서대로 나열
// [ ] 대괄호 안에 , 쉼표 이용하여 여러개 자료들을 *하나로 자료*로 표현
...
// 5. 배열 요소 찾기 , .indexof( 찾을값 ), 만약에 찾을값이 존재하면 찾은 인덱스 반환 없으면 -1
...
// [6] 타입 변환,    3 vs "3" 숫자3과 문자3임 서로 다르다 , 실무는 JS만으로만 개발 X JS+JAVA+PYTHON+DATA+EXCEL+TEXT 등 외부언어/자료 혼합O
// 서로 다른 언어/자료들간의 통산 HTTP 통신한다 JSON/XML/CSV 타입 ( 문자열 )
// 1. 자동 타입 변환
console.log( 3 + 3.14 )          // 정수 + 실수 = > 실수(숫자)
console.log( '유재석' + 40 )      // 문자 + 숫자 => 문자
console.log( 3 + true )         // 정수 + 논리 => 숫자(숫자) 4 논리가 숫자 취급됨
// 2. 직접 타입 변환
...
console.log( 10 + "" )           // 빠르게 숫자형 10을 문자열 10으로 변환
