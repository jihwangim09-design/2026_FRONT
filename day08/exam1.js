...
const person = { 'name' : '김지환', 'age' : "25" , 'city' : "서울" 
}
console.log ( person )
*/
...
//console.log ( Object.values(book) ) //객체내 모든 속상값 (배열)반환
...
console.log( student.address.city ) // 방법1
...
let 상품명 = Object.keys( inventory );
let 재고량 = Object.values( inventory );

for ( i = 0 ; i<=재고량.length - 1 ; i++) {
    if (재고량[i] < 10) 
            console.log (상품명[i])
ㄱㅅㄴ
let 속성명들 = Object.keys( inventory );

for ( index = 0 ; i<=속성명들.length - 1 ; index++) {
