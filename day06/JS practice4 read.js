...
let max = numbers[0] ; // 배열명[인덱스] : 특정한 인덱스 번째 값 요소 호출 // 첫번째 값ㅇ르 최댓값 지정
...
if ( max < numbers[3] ){ max = numbers[4] } //  [4]
if ( max < numbers[3] ){ max = numbers[5] } //  [5]
console.log ( max )
// vs 반복문 풀이
// 반복되는 코드 : if ( max < numbers[?] ){ max = numbers[?] }
// 반복없는 코드  ( 패턴: 1부터 5까지 1씩증가 )
let numbers = [23, 5, 67, 12, 88, 34];
let max = numbers[0]
for ( let index =  1 ; index <= 5 ; i++ ) {
...
// star는 1부터 (마지막줄수-현재줄수+1) 1씩 증가하면서 '*'출력
...
/*문제 7: 재고 관리 시스템
두 개의 배열 products(상품 목록)와 stock(재고 수량)이 있습니다. 사용자로부터 구매할 상품명과 수량을 입력받아, 재고가 충분하면 "구매 완료!"를 출력하고 재고를 차감하세요. 재고가 부족하면 "재고가 부족합니다."를 출력합니다. * 배열 includes 함수 사용 시 for문 사용 안 함.
